...
class RadiusInstance:
	...
	def _post(self, module, payload, url_append='', parameters=None):
		...
		u = self.server_url + self._get_module_name(module) + '/' + url_append + '?' + urlencode(p)
	...
	def update_entity(self, module, entity_id, request_body):
		...
		consisting of createFields and, optionally, returnFields. Returns an
		entity ID, or other requested fields."""
		if module == 'Registrations' and ('Participant' not in request_body['createFields'].keys() or 'Iteration Name' not in request_body['createFields'].keys()):
			...
			registration_ro = self.create_request_object('Registrations', {'Entity ID': str(entity_id)}, type='search', return_fields=['Participant', 'Iteration Name'])
			registration = self.search_for_entities('Registrations', registration_ro)
	...
	def search_for_entities(self, module, request_body):
		...
		e = self._post(module=module, url_append='search', payload=request_body)
		if e['total pages'] == 1:
			return e['entities']
		else:
	...
	def get_export_filter_as_list(self, task_id):
		...
		t = self._get(module='ExportFilters', url_append='getExecutionTaskResult/'+task_id)
	...
	def get_export_filter_id_by_name(self, export_filter_name):
		...
		search_object = self.create_request_object('ExportFilters', {'Filter Name': export_filter_name}, type='search', return_fields=['Entity ID'])
		id = self.search_for_entities('ExportFilters', search_object)
		if id:
	...
	def create_request_object(self, module, fields, type='search', return_fields=[], strict=False):
		...
		for field in fields.keys():
					...
					if field == a or field == all_fields[a]['Display Label']:
						if 'Possible Values' in all_fields[a].keys():
							if fields[field] in all_fields[a]['Possible Values']:
								...
								raise APIError(f'Field value <{fields[field]}> not found in possible values for field <{field}>.')
						else:
							checked_fields[a] = fields[field]
							positive_id = True
			...
			if not positive_id and strict:
				raise APIError(f'Field name <{field}> not found in module <{module}>.')
